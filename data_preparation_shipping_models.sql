...
WITH cleaned AS (
    SELECT DISTINCT
        ...
        s.transaction_date,
        s.customer_id,
        COALESCE(p.description, s.description) AS description_cleaned,
        p.category,
...
base AS (
    SELECT
        ...
        category,
        order_city_cleaned AS order_city,
		state,
        order_postal,
        region,
        latitude,
    ...
    FROM cleaned
    WHERE quantity_cleaned > 0 
		AND COALESCE(costs,0) > 0
)
...
SELECT
   ...
   CASE
    ...
    END AS hypothesis_shipping_costs,
